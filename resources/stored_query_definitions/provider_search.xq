...
<CSD xmlns:csd="urn:ihe:iti:csd:2013"  >
  ...
  <providerDirectory>
	...
	then csd_bl:filter_by_languages($provs7,$careServicesRequest/language)
      ...
      let $provs9 := if(exists($careServicesRequest/region))
      	then
      		let $districts := data(/CSD/organizationDirectory/organization[./codedType/@code='DISTRICT' and ./organizations/organization[@entityID=data($careServicesRequest/region/@entityID)]]/@entityID)
      		return $provs8[./organizations/organization[@entityID = $districts]]
      	else $provs8


      return if (exists($careServicesRequest/start)) then
	if (exists($careServicesRequest/max)) 
	  then csd_bl:limit_items($provs9,$careServicesRequest/start,$careServicesRequest/max)         
	else csd_bl:limit_items($provs9,$careServicesRequest/start,<max/>)         
      else
	if (exists($careServicesRequest/max)) 
	  then csd_bl:limit_items($provs9,<start/>,$careServicesRequest/max)         
	else $provs9
